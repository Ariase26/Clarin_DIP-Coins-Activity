...
namespace ImageProcess2
{
    ...
    public class Coins
    {
        ...
        public static void CountCoin(Bitmap bmp, ref Label countLabel, ref Label valueLabel,
                                      ref Label fivecent, ref Label tencent, ref Label twofivecent,
                                      ref Label onepeso, ref Label fivepeso)
        {
            int count = 0;
            float value = 0;

            int count5c = 0, count10c = 0, count25c = 0, count1p = 0, count5p = 0;
            float value5c = 0, value10c = 0, value25c = 0, value1p = 0, value5p = 0;
            ...
            for (int i = 0; i < height; i++)
            {
                for (int j = 0; j < width; j++)
                {
                    ...
                    {
                        count++;
                        int pixelCount = BFS(bmp, ref visited, j, i);
                        float coinValue = Classify(pixelCount, ref count);
                        ...
                        else if (coinValue == PESO_1)
                        {
                            count1p++;
                            value1p += PESO_1;
                            value += PESO_1;
                        }
                        else if (coinValue == CENT_25)
                        {
                            count25c++;
                            value25c += CENT_25;
                            value += CENT_25;
                        }
                        else if (coinValue == CENT_10)
                        {
                            count10c++;
                            value10c += CENT_10;
                            value += CENT_10;
                        }
                        else if (coinValue == CENT_5)
                        {
                            count5c++;
                            value5c += CENT_5;
                            value += CENT_5;
            ...
            fivecent.Text = $"{count5c} coins (P{value5c:F2})";
            tencent.Text = $"{count10c} coins (P{value10c:F2})";
            twofivecent.Text = $"{count25c} coins (P{value25c:F2})";
            onepeso.Text = $"{count1p} coins (P{value1p:F2})";
            fivepeso.Text = $"{count5p} coins (P{value5p:F2})";
        }

        private static bool IsBlack(Color pixel, int threshold = 10)
        {
        ...
        {
            Debug.WriteLine($"Pixel Count: {pixel}");
        ...
        private static int BFS(Bitmap bmp, ref bool[,] visited, int x, int y)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                if (IsValidPixel(bmp, visited, dx - 1, dy)) queue.Enqueue(new Point(dx - 1, dy));
                if (IsValidPixel(bmp, visited, dx + 1, dy)) queue.Enqueue(new Point(dx + 1, dy));
                if (IsValidPixel(bmp, visited, dx, dy - 1)) queue.Enqueue(new Point(dx, dy - 1));
                if (IsValidPixel(bmp, visited, dx, dy + 1)) queue.Enqueue(new Point(dx, dy + 1));
            }
        ...
        private static bool IsValidPixel(Bitmap bmp, bool[,] visited, int x, int y)
        {
            return x >= 0 && x < bmp.Width && y >= 0 && y < bmp.Height &&
                   !visited[y, x] && IsBlack(bmp.GetPixel(x, y));
        }
    }
